...
class Config:
    ...
    # Use absolute path if it's just a filename, otherwise use as-is
    if os.path.isabs(_creds_path):
        GOOGLE_CREDENTIALS_PATH = _creds_path
    else:
        GOOGLE_CREDENTIALS_PATH = os.path.join(os.path.dirname(__file__), _creds_path)
...
async def download_results_json(key: str):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=results_{key}.json"
        }
    )
